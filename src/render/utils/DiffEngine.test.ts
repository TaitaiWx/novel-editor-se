...
import { diff, lineDiff } from './DiffEngine';
...
export function runPerformanceTests() {
  ...
  console.log(
    `小文件结果: ${smallResult.operations.length} 个操作, ${smallResult.totalChanges} 个变化`
  );
  ...
  console.log(
    `中等文件结果: ${mediumResult.operations.length} 个操作, ${mediumResult.totalChanges} 个变化`
  );
  ...
  console.log(
    `大文件结果: ${largeResult.operations.length} 个操作, ${largeResult.totalChanges} 个变化`
  );
  ...
  console.log(
    `超大文件结果: ${hugeResult.operations.length} 个操作, ${hugeResult.totalChanges} 个变化`
  );
  ...
  console.log(
    `行级diff结果: ${lineResult.operations.length} 个操作, ${lineResult.totalChanges} 个变化`
  );
}
...
export function runCorrectnessTests() {
  ...
  console.log(
    '相同文本测试:',
    sameResult.operations.length === 1 && sameResult.operations[0].type === 'equal'
      ? '通过'
      : '失败'
  );
  ...
  console.log(
    '添加内容测试:',
    addResult.operations.length === 2 && addResult.operations[1].type === 'add' ? '通过' : '失败'
  );
  ...
  console.log(
    '删除内容测试:',
    deleteResult.operations.length === 2 && deleteResult.operations[1].type === 'delete'
      ? '通过'
      : '失败'
  );
