...
function configureAutoUpdater(updater: AppUpdater, channel: UpdateChannel) {
  const mappedChannel = mapUpdateChannel(channel);
...
function resetStartupHealthState() {
...
async function tryMarkVersionHealthy() {
...
function armHealthyStartupObservers() {
...
export async function getUpdateStatus() {
  ...
  updaterStatus.rollbackVersion = state.rollbackTarget?.version ?? null;
  updaterStatus.pendingVersion = state.pendingVersion;
  if (autoUpdaterUnavailableReason && !updaterStatus.lastError) {
